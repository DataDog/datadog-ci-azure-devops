...
| `failOnMissingTests`   | _optional_  | Whether to fail the CI job if at least one test is missing in a run (for example, if it has been removed or deleted). **Default:** `false`.                                                                                                     |
| `failOnCriticalErrors` | _optional_  | Whether to fail the CI job if no tests were triggered, or results could not be fetched from Datadog. **Default:** `false`.                                                                                                                      |
